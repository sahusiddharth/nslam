...
from env.habitat.utils.attention.gradient import SmoothGradCAMpp
from env.habitat.utils.attention.activation import CAM
import matplotlib.pyplot as plt
from torchvision.transforms.functional import normalize, resize, to_pil_image
from env.habitat.utils.attention.utils import overlay_mask
...
'''
Sets the boundary in form of a matrix 
for the local map based on the agent's
location and the full map.
'''
...
thread1_path = "/home/mangesh_singh/Desktop/Project/Neural-SLAM/cam_output_thread1"
thread2_path = "/home/mangesh_singh/Desktop/Project/Neural-SLAM/cam_output_thread2"
iter_num = 0

def update_tensor(obs):
    global thread1_path, thread2_path, iter_num
    arr = obs.numpy()
    result1 = mark_importance(arr[0])
    result2 = mark_importance(arr[1])
    plt.imshow(np.transpose(result1, (1, 2, 0)) / 255.0)
    plt.savefig(f"{thread1_path}/{iter_num}.png")
    plt.close()
    plt.imshow(np.transpose(result1, (1, 2, 0)) / 255.0)
    plt.savefig(f"{thread2_path}/{iter_num}.png")
    plt.close()
    iter_num += 1
...
def mark_importance(input):
    cam_model = resnet18(pretrained=True).eval()
    ...
    activation_image = to_pil_image(activation_map[0].squeeze(0), mode='F')
    activation_image_resized = resize(activation_image, (input.shape[1], input.shape[2]))
    activation_array = np.array(activation_image_resized)
    threshold = np.percentile(activation_array, 40) 
    mask = np.zeros_like(activation_array)
    for i in range(len(activation_array)):
        for j in range(len(activation_array[i])):
            if activation_array[i][j] > threshold:
                mask[i][j] = activation_array[i][j]
    
    # Ensure mask has the same number of channels as the input
    mask = np.stack([mask] * 3, axis=0)
    important_patch = input * mask

    return important_patch

def main():
    """ Defining Region Importance """
